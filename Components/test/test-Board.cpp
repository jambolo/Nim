...
TEST(Board, Constructor)
{
    ...
    ASSERT_NO_THROW(Board(std::vector<int>(Board::MAX_HEAPS, 1)));                          // Maximum number of heaps
    EXPECT_DEATH(Board({}), "Assertion failed: .*");                                        // Board with no heaps
    EXPECT_DEATH(Board(std::vector<int>(Board::MAX_HEAPS + 1, 1)), "Assertion failed: .*"); // Too many heaps
