...
TEST(ZHash, Constructor_board_player)
{
    ...
    // Just to be thorough and knowing that every empty heap has a hash value of 0, we should check all permutations except all empty.
    {
        Board boards[] =
        {
            Board({ 0, 0, 1 }),
        ...
        std::transform(std::begin(boards), std::end(boards), std::back_inserter(hashes),
            [](const Board& board) {
                return ZHash(board, NimState::PlayerId::FIRST);
            });
...
TEST(ZHash, ChangeHeap)
{
    ...
    // Make a bunch of changes to heaps and then undo them in the same order. All hashes should be different, but the final hash should be the same as the initial one.
    std::tuple<int, int, int> changes[] =
    {
        {0, 0, 1},
        {0, 1, 2},
        {1, 0, 1},
    ...
    EXPECT_FALSE(std::any_of(hashes.begin(), hashes.end(),
        [&z0](ZHash const & hash) { return hash.value() == z0.value(); })); // None of the hashes should be the same as the initial one
}
